...
class CartController extends Controller
{
    ...
    protected function getCartIdentifier() {
        if (Auth::check()) {
            ...
            if (!session()->has('guest_cart_id')) {
                session()->put('guest_cart_id', Str::random(32));
            }
            return ['session_id' => session('guest_cart_id')];
        }
    }
    ...
    public function addToCart(Request $request) {
        ...
        if($already_cart) {
            ...
            // Set identifier based on auth status
            if (Auth::check()) {
                $cart->user_id = auth()->user()->id;
            } else {
                $cart->session_id = $cartIdentifier['session_id'];
            }
            ...
            // Handle wishlist only for authenticated users
            if (Auth::check()) {
    ...
    public function cartDelete(Request $request) {
        $cartIdentifier = $this->getCartIdentifier();
        $cart = Cart::where($cartIdentifier)->find($request->id);
    ...
    public function cartUpdate(Request $request) {
        if($request->quant) {
            ...
            foreach ($request->quant as $k=>$quant) {
                $id = $request->qty_id[$k];
                $cart = Cart::where($cartIdentifier)->find($id);
    ...
    protected function mergeGuestCart() {
        $guestCartItems = Cart::where('session_id', session('guest_cart_id'))
            ->where('order_id', null)
            ->get();
        ...
        foreach ($guestCartItems as $guestItem) {
            ...
            if ($existingCart) {
                ...
                $existingCart->amount += $guestItem->amount;
                $existingCart->save();
                $guestItem->delete();
            } else {
                // Convert guest cart item to user cart item
                $guestItem->user_id = auth()->user()->id;
    ...
    public function addToCartXXX(Request $request){
        // dd($request->all());
        if (empty($request->slug)) {
            request()->session()->flash('error','Invalid Products');
            return back();
    ...
    public function singleAddToCartXXX(Request $request){
        $request->validate([
            'slug'      =>  'required',
            'quant'      =>  'required',
        ]);
    ...
    public function cartDeleteXXX(Request $request){
    ...
    public function cartUpdateXXX(Request $request){
        // dd($request->all());
        if($request->quant){
            $error = array();
            $success = '';
    ...
    public function checkoutXXX(Request $request){
        // $cart=session('cart');
